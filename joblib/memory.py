...
class MemorizedResult(Logger):
    ...
    def __reduce__(self):
        return (self.__class__,
                (self.store, self.func, self.args_id),
                {'mmap_mode': self.mmap_mode})
...
class MemorizedFunc(Logger):
    ...
    def call_and_shelve(self, *args, **kwargs):
        ...
        return MemorizedResult(self.store, self.func, args_id,
    ...
    def __reduce__(self):
        ...
        """
        return (self.__class__, (self.func, self.store, self.ignore,
    ...
    def _get_output_idendifiers(self, *args, **kwargs):
        ...
        return func_identifier, argument_hash
    ...
    def __repr__(self):
        return ("{0}(func={1}, location={2})"
                .format(self.__class__.__name__, self.func, self.store,))
...
class Memory(Logger):
    ...
    def cache(self, func=None, ignore=None, verbose=None, mmap_mode=False):
        ...
        if isinstance(func, MemorizedFunc):
            func = func.func
        return MemorizedFunc(func, self.store, mmap_mode=mmap_mode,
    ...
    def __repr__(self):
        return '{0}(location={1})'.format(self.__class__.__name__,
                                          (repr(None) if self.store is None
                                           else repr(self.store)))
    ...
    def __reduce__(self):
        ...
        location = repr(self.store)[:-7] if self.store is not None else None
        compress = self.store.compress if self.store is not None \
            else False
        return (self.__class__, (location, self.mmap_mode, compress,
                                 self._verbose))
